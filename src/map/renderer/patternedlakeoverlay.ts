 ...
 *                                      It is handed straight to d3 as the attribute callback, so it is
 *                                      called with the geoJson, d3's index and the group.
 ...
 * @property {String, Function} lakePathColor  The stroke colour of those borders. It has no default, and is applied only
 *                                      when truthy, so the empty string cannot clear a colour already set; see the note
 *                                      below. Not wrapped in fn.functor.
 ...
 *                                      default branch is the one no in-repo chart takes. Turning it off after a render
 *                                      with it on does not undo the fade; see the note below.
 *
 * Note: every render calls the pattern helpers again on the same defs elements, and each helper
 * appends its contents unconditionally rather than joining them - so the tile gains another rect
 * and another two lines, the fade gradient another two stops, and the mask another rect on every
 * redraw. A map that re-renders on resize or on a control change grows these definitions without
 * bound. The base and geojson renderers call their own pattern helper the same way.
 *
 ...
 * geojson renderers' "missing-pattern". The gradient's id is written twice over, once by
 * ensureDefsElement and once by the gradient helper itself.
 *
 ...
 * inline style beats any author rule short of !important. The mesh's borderColor has the same shape.
 *
 ...
 * same group rebinds the first one's paths instead of drawing its own. One overlay per layer.
 *
 * Note: no transition is scheduled, and the path data is reapplied on every render rather than only
 * on enter, so a geoJson mutated in place still repaints. This component adds no tooltip anchors
 * and no event targets. See test/map/renderer/patternedlakeoverlay.test.ts.
 *
