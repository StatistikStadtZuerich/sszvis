...
## sszvis.groupedBarsVertical

Also available as `sszvis.groupedBars()` for backward compatibility. For horizontal grouped bars, see `sszvis.groupedBarsHorizontal()`.
...
#### `groupedBarsVertical.groupScale(scale)`
...
#### `groupedBarsVertical.height(height)`
