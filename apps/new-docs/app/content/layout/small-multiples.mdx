...
<Example id="line-chart/line-overlapping" />

### Example with small multiples

<Example id="line-chart/line-multiples" />
...
const chartGroups = chart.selectGroup("multiples").datum(groupedData).call(multiplesMaker);
...
chartGroups.selectAll(".sszvis-multiple").each(function (d) {
  ...
  group.selectAll(".sszvis-multiple-chart").datum(d.values).call(chartComponent);
});
```
...
<Example id="pie-charts/pie-multiples" />
