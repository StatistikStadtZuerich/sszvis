/**
 * Choropleth map of the twelve city districts (Kreise) of Zurich.
 *
...
const queryProps = sszvis
  ...
  .prop("labelFormat", { _: () => sszvis.formatNumber })
  .prop("tooltipUnit", { _: "Einwohner" });
...
sszvis.app<State, Actions>({
  ...
  actions: {
    selectHovered(state, _e, d) {
      if (d.datum === undefined || state.selection[0] === d.datum) {
        return;
      }
  ...
  render(state, actions) {
    ...
    const chartLayer = sszvis
      .createSvgLayer(config.id, bounds, {
        title: "Bevölkerung nach Stadtkreis",
        description: "Anzahl Einwohnerinnen und Einwohner je Stadtkreis der Stadt Zürich.",
      })
      .datum(state.data);
    ...
    const tooltipBody = sszvis
      .modularTextHTML()
      .plain((d: MapDatum) => (d.datum === undefined ? "" : sszvis.formatNumber(vAcc(d.datum))))
