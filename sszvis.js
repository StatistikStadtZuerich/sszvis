 ...
 */
var flatten = function(arr) { return Array.prototype.concat.apply([], arr); };
 ...
 *                              of touches.
 */
var firstTouch = function(event$$1) {
  if (event$$1.touches && event$$1.touches.length) {
    return event$$1.touches[0];
  } else if (event$$1.changedTouches && event$$1.changedTouches.length) {
    return event$$1.changedTouches[0];
  }
  return null;
};
...
var textWrap = function(selection$$1, width, paddingRightLeft, paddingTopBottom) {
 ...
 */
var range$1 = function(scale) { // borrowed from d3 source - svg.axis
  return scale.rangeExtent ? scale.rangeExtent() : extent$1(scale.range());
};
 ...
 */
function extent$1(domain) { // borrowed from d3 source - svg.axis
...
function component() {
   ...
   */
  function sszvisComponent(selection$$1) {
    if (selectionRenderer) {
      selection$$1.props = function(){ return clone(props); };
      selectionRenderer.apply(selection$$1, slice$1(arguments));
    }
    selection$$1.each(function() {
...
var axis = function() {
  ...
  var axisComponent = component()
    ...
    .render(function() {
      var selection$$1 = d3.select(this);
      var props = selection$$1.props();
      ...
      ['scale', 'ticks', 'tickValues', 'tickSizeInner',
       'tickSizeOuter', 'tickPadding', 'tickFormat', 'tickSize'].forEach(function(prop$$1) {
        if (props[prop$$1] !== undefined) {
          if (axisDelegate[prop$$1] === undefined) {
            throw new Error('axis: "' + prop$$1 + '" not available');
          }
          axisDelegate[prop$$1](props[prop$$1]);
        }
      });
      ...
      var group = selection$$1.selectGroup('sszvis-axis')
      ...
      var rangeExtent = range$1(axisScale);
      ...
      if (props.alignOuterLabels) {
        var alignmentBounds = range$1(axisScale);
        var min$$1 = alignmentBounds[0];
        var max$$1 = alignmentBounds[1];

        tickTexts
          .style('text-anchor', function(d) {
            var value = axisScale(d);
            if (absDistance(value, min$$1) < TICK_END_THRESHOLD) {
              return 'start';
            } else if (absDistance(value, max$$1) < TICK_END_THRESHOLD) {
              return 'end';
            }
            return 'middle';
          });
      ...
      if (props.slant) {
        tickTexts
          .call(slantLabel[props.orient][props.slant]);
      }
        ...
        title.merge(newTitle)
          ...
          .attr('transform', function() {
            var orient = props.orient,
              axisScaleExtent = range$1(axisScale),
              titleProps;
...
var slantLabel = {
  top: {
    horizontal: function(selection$$1) {
      selection$$1.style('text-anchor', 'middle')
        .attr('dx', "-0.5")
        .attr('transform', null);
    },
    vertical: function(selection$$1) {
      selection$$1.style('text-anchor', 'start')
        .attr('dx', '0em')
        .attr('dy', '0.35em') // vertically-center
        .attr('transform', 'rotate(-90)');
    },
    diagonal: function(selection$$1) {
      selection$$1.style('text-anchor', 'start')
        .attr('dx', '0.1em')
        .attr('dy', '0.1em')
  ...
  bottom: {
    ...
    vertical: function(selection$$1) {
      selection$$1.style('text-anchor', 'end')
        .attr('dx', '-1em')
        .attr('dy', '-0.75em')
        .attr('transform', 'rotate(-90)');
    },
...
function bounds(arg1 /* bounds or selection */, arg2 /* [selection] */) {
  var _bounds = null, selection$$1 = null;
  ...
  var dimensions = defined(selection$$1) ? measureDimensions(selection$$1) : {width: DEFAULT_WIDTH};
...
var scaleQual12 = qualColorScale([
  '#5182B3', '#B8CFE6',
  '#60BF97', '#B8E6D2',
  '#94BF69', '#CFE6B8',
  '#E6CF73', '#FAEBAF',
...
var scaleQual6 = qualColorScale([
  '#5182B3', '#60BF97',
  '#94BF69', '#E6CF73',
  '#E67D73', '#CC6788'
]);
var scaleQual6a = qualColorScale([
  '#5182B3', '#B8CFE6',
  '#60BF97', '#B8E6D2',
  '#94BF69', '#CFE6B8'
]);
var scaleQual6b = qualColorScale([
  '#E6CF73', '#FAEBAF',
  '#E67D73', '#F2CEC2',
  '#CC6788', '#E6B7C7'
]);
...
function decorateOrdinalScale(scale) {
  scale.darker = function(){
    return decorateOrdinalScale(
      scale.copy().range(scale.range().map(func$1('darker', LIGHTNESS_STEP)))
    );
  };
  scale.brighter = function(){
    return decorateOrdinalScale(
      scale.copy().range(scale.range().map(func$1('brighter', LIGHTNESS_STEP)))
    );
  };
...
function createSvgLayer(selector, bounds$$1, metadata) {
  ...
  svg.merge(svgEnter)
    .attr('height', bounds$$1.height)
    .attr('width',  bounds$$1.width);
  ...
  viewport.merge(viewportEnter)
    .attr('transform', 'translate(' + (bounds$$1.padding.left) + ',' + (bounds$$1.padding.top) + ')');
...
var fallbackRender = function(selector, options) {
  ...
  var selection$$1 = isSelection(selector) ? selector : d3.select(selector);
  selection$$1.append('img')
...
var move = function() {
  var event$$1 = d3.dispatch('start', 'move', 'drag', 'end');
  ...
  var moveComponent = component()
    ...
    .prop('yScale')
    .prop('draggable')
    .prop('cancelScrolling', functor).cancelScrolling(false)
    .prop('fireOnPanOnly', functor).fireOnPanOnly(false)
    .prop('padding', function(p) {
      var defaults = { top: 0, left: 0, bottom: 0, right: 0 };
      for (var prop$$1 in p) { defaults[prop$$1] = p[prop$$1]; }
      return defaults;
    }).padding({})
    .render(function() {
      ...
      var xExtent = range$1(props.xScale).sort(d3.ascending);
      var yExtent = range$1(props.yScale).sort(d3.ascending);
      ...
      var layer = selection$$1.selectAll('[data-sszvis-behavior-move]')
        .data([0]);
      ...
      layer
        ...
        .on('mouseover', function() {
          event$$1.apply('start', this, arguments);
        })
        .on('mousedown', function() {
          ...
          var stopDragging = function() {
            ...
            event$$1.apply('end', this, arguments);
          };
        ...
        .on('mousemove', function() {
          ...
          if (!target.__dragging__) {
            event$$1.apply('move', this, [x, y]);
          }
        })
        .on('mouseout', function() {
          event$$1.apply('end', this, []);
        })
          ...
          if (!props.fireOnPanOnly() || cancelScrolling) {
            ...
            var pan = function() {
              ...
              if (!props.fireOnPanOnly() || panCancelScrolling) {
                event$$1.apply('drag', this, [panX, panY]);
                event$$1.apply('move', this, [panX, panY]);
              } else {
                event$$1.apply('end', this, []);
              }
            };
  ...
  moveComponent.on = function() {
    ...
    return value === event$$1 ? moveComponent : value;
  };
...
var datumFromPannableElement = function(element) {
  if (!isNull(element)) {
    var selection$$1 = d3.select(element);
    if (!isNull(selection$$1.attr('data-sszvis-behavior-pannable'))) {
      var datum = selection$$1.datum();
      if (defined(datum)) {
        return datum;
      }
    }
...
var panning = function() {
  var event$$1 = d3.dispatch('start', 'pan', 'end');
  ...
  var panningComponent = component()
    ...
    .render(function() {
      ...
      var elements = selection$$1.selectAll(props.elementSelector);
      ...
      elements
        ...
        .on('mousemove', function() {
          event$$1.apply('pan', this, arguments);
        })
  ...
  panningComponent.on = function() {
    var value = event$$1.on.apply(event$$1, arguments);
    return value === event$$1 ? panningComponent : value;
  };
...
var voronoi$1 = function() {
  var event$$1 = d3.dispatch('over', 'out');
  ...
  var voronoiComponent = component()
    ...
    .render(function(data) {
      ...
      var voronoi$$1 = d3.voronoi()
        .x(props.x)
        .y(props.y)
        .extent(props.bounds);

      var polys = selection$$1.selectAll('[data-sszvis-behavior-voronoi]')
        .data(voronoi$$1.polygons(data));
      ...
      polys
        ...
        .on('mouseover', function(datum) {
          ...
          if (eventNearPoint(d3.event, [cbox.left + props.x(datum.data), cbox.top + props.y(datum.data)])) {
            event$$1.apply('over', this, [datum.data]);
          }
        })
            ...
            var pan = function() {
              ...
              if (panDatum !== null) {
                ...
                if (eventNearPoint(touchEvent, [panCbox.left + props.x(panDatum.data), panCbox.top + props.y(panDatum.data)])) {
                  ...
                  event$$1.apply('over', this, [panDatum.data]);
                } else {
                  event$$1.apply('out', this, []);
                }
              } else {
  ...
  voronoiComponent.on = function() {
    var value = event$$1.on.apply(event$$1, arguments);
    return value === event$$1 ? voronoiComponent : value;
  };
...
function eventNearPoint(event$$1, point) {
  var dx = event$$1.clientX - point[0];
  var dy = event$$1.clientY - point[1];
...
var heatTableDimensions = function(spaceWidth, squarePadding, numX, numY, chartPadding) {
...
var horizontalBarChartDimensions = function(numBars) {
...
var verticalBarChartDimensions = function(width, numBars) {
...
var populationPyramidLayout = function(spaceWidth, numBars) {
...
var smallMultiples = function() {
  return component()
    ...
    .render(function(data) {
      ...
      var multiples = selection$$1.selectAll('g.sszvis-multiple')
        .data(data);
...
var stackedAreaMultiplesLayout = function(height, num, pct) {
  pct || (pct = 0.1);
...
var prepareData$1 = function() {
  ...
  function main(data) {
    ...
    var root = d3.hierarchy({ isSunburstRoot: true, values: nester.entries(data) }, prop('values'))
      .sort(sortFn)
    ...
    function flatten$$1(node) {
      return [].concat.apply([node], (node.children || []).map(flatten$$1));
    }

    // Remove the root element from the data (but it still exists in memory so long as the data is alive)
    return flatten$$1(root).filter(function(d) { return !d.data.isSunburstRoot; });
  }
...
var ensureDefsElement = function(selection$$1, type, elementId) {
  var element = ensureDefsSelection(selection$$1)
 ...
 */
function ensureDefsSelection(selection$$1) {
  var defs = selection$$1.selectAll('defs')
    .data([0]);
...
function formatHTML() {
  ...
  return function(textBody, datum) {
    return textBody.lines().map(function(line$$1) {
      return line$$1.map(function(word) {
...
function formatSVG() {
  ...
  return function(textBody, datum) {
    return textBody.lines().reduce(function(svg, line$$1, i) {
      var lineSvg = line$$1.map(function(word) {
        return styles[word.style].call(null, word.text(datum));
      }).join(' ');
      var dy = (i === 0) ? 0 : '1.2em';
...
function createHtmlLayer(selector, bounds$$1, metadata) {
  ...
  layer
    .style('position', 'absolute')
    .style('left', bounds$$1.padding.left + 'px')
    .style('top', bounds$$1.padding.top + 'px');
...
var buttonGroup = function() {
  return component()
    ...
    .render(function() {
      ...
      var container = selection$$1.selectAll('.sszvis-control-optionSelectable')
...
var handleRuler = function() {
  return component()
    .prop('x', functor)
    .prop('y', functor)
    .prop('top')
    .prop('bottom')
    .prop('label').label(functor(''))
    .prop('color')
    .prop('flip', functor).flip(false)
    .render(function(data) {
      var selection$$1 = d3.select(this);
      var props = selection$$1.props();
      ...
      var group = selection$$1.selectAll('.sszvis-handleRuler__group')
        .data([0]);
      ...
      var labelOutline = selection$$1.selectAll('.sszvis-ruler__label-outline')
        .data(data);
      ...
      var label = selection$$1.selectAll('.sszvis-ruler__label')
        .data(data);
      ...
      selection$$1.selectAll('.sszvis-ruler__label, .sszvis-ruler__label-outline')
        .attr('transform', function(d) {
...
var select$1 = function() {
  return component()
    ...
    .render(function() {
      ...
      var wrapperEl = selection$$1.selectAll('.sszvis-control-optionSelectable')
...
var slider = function() {
  return component()
    ...
    .prop('tickLabels', functor).tickLabels(identity)
    .prop('label', functor).label(identity)
    .render(function() {
      var selection$$1 = d3.select(this);
      var props = selection$$1.props();
      ...
      var scaleRange = range$1(props.scale);
      ...
      var bg = selection$$1.selectAll('g.sszvis-control-slider__backgroundgroup')
        .data([1]);
      var newBg = bg.enter()
        .append('g')
      ...
      var axis = axisX()
        ...
        .tickPadding(6)
        .tickValues(set([].concat(props.majorTicks, props.minorTicks)))
        .tickFormat(function(d) {
      ...
      shadow
        .attr('x1', Math.ceil(scaleRange[0] + lineEndOffset))
        .attr('x2', compose(Math.floor, alteredScale));

      // draw the handle and the label
      var handle = selection$$1.selectAll('g.sszvis-control-slider__handle')
        .data([props.value]);
      handle.exit().remove();
      ...
      selection$$1.selectGroup('sliderInteraction')
...
var heatTableMissingValuePattern = function(selection$$1) {
...
var mapMissingValuePattern = function(selection$$1) {
  var pWidth = 14,
      pHeight = 14,
      fillColor = '#FAFAFA',
      lineStroke = '#CCCCCC';
...
var mapLakePattern = function(selection$$1) {
  var pWidth = 6;
  var pHeight = 6;
  var offset = 0.5;
...
var mapLakeFadeGradient = function(selection$$1) {
  selection$$1
    .attr('x1', 0)
    .attr('y1', 0)
    .attr('x2', 0.55)
...
var mapLakeGradientMask = function(selection$$1) {
  selection$$1
...
var dataAreaPattern = function(selection$$1) {
  var pWidth = 6;
  var pHeight = 6;
  var offset = 0.5;
...
var circle = function() {
  return component()
    ...
    .prop('dx', functor)
    .prop('dy', functor)
    .prop('caption', functor)
    .render(function(data) {
      var selection$$1 = d3.select(this);
      var props = selection$$1.props();

      ensureDefsElement(selection$$1, 'pattern', 'data-area-pattern')
        .call(dataAreaPattern);

      var dataArea = selection$$1.selectAll('.sszvis-dataareacircle')
        .data(data);
      ...
      if (props.caption) {
        var dataCaptions = selection$$1.selectAll('.sszvis-dataareacircle__caption')
          .data(data);
...
var line$1 = function() {
  return component()
    ...
    .prop('xScale')
    .prop('yScale')
    .prop('dx', functor).dx(0)
    .prop('dy', functor).dy(0)
    .prop('caption', functor)
    .render(function(data) {
      var selection$$1 = d3.select(this);
      var props = selection$$1.props();
      ...
      var line$$1 = selection$$1.selectAll('.sszvis-referenceline')
        .data(data);

      line$$1.exit().remove();

      var newLine = line$$1.enter()
        .append('line')
        .classed('sszvis-referenceline', true);

      line$$1 = line$$1.merge(newLine);
      ...
      if (props.caption) {
        var caption = selection$$1.selectAll('.sszvis-referenceline__caption')
          .data([0]);
...
var tooltipAnchor = function() {

  return component()
    .prop('position').position(functor([0, 0]))
    .prop('debug')
    .render(function(data) {
      var selection$$1 = d3.select(this);
      var props = selection$$1.props();

      var anchor = selection$$1.selectAll('[data-tooltip-anchor]')
        .data(data);
      ...
      anchor
        .attr('transform', compose(vectorToTranslateString, props.position));


      // Visible anchor if debug is true
      if (props.debug) {
        var referencePoint = selection$$1.selectAll('[data-tooltip-anchor-debug]')
          .data(data);
...
var rangeFlag = function() {
  return component()
    ...
    .render(function(data) {
      ...
      selection$$1.selectAll('.sszvis-rangeFlag__mark.bottom')
        .data(data)
        .call(makeFlagDot('bottom', crispX, crispY0));

      selection$$1.selectAll('.sszvis-rangeFlag__mark.top')
        .data(data)
...
var rangeRuler = function() {
  return component()
    ...
    .render(function(data) {
      ...
      var line$$1 = selection$$1.selectAll('.sszvis-rangeRuler__rule')
        .data([0]);
      ...
      var marks = selection$$1.selectAll('.sszvis-rangeRuler--mark')
        .data(data);
      ...
      var total = selection$$1.selectAll('.sszvis-rangeRuler__total')
        .data([last(data)]);
...
var rectangle = function() {
  return component()
    ...
    .render(function(data) {
      ...
      var dataArea = selection$$1.selectAll('.sszvis-dataarearectangle')
        .data(data);
      ...
      if (props.caption) {
        var dataCaptions = selection$$1.selectAll('.sszvis-dataarearectangle__caption')
          .data(data);
...
var ruler = function() {
  ...
  return component()
    ...
    .render(function(data) {
      ...
      var ruler = selection$$1.selectAll('.sszvis-ruler__rule')
        .data(data, labelId);
      ...
      ruler
        .attr('x1', compose(halfPixel, props.x))
        .attr('y1', props.y)
        .attr('x2', compose(halfPixel, props.x))
        .attr('y2', props.bottom);


      var dot = selection$$1.selectAll('.sszvis-ruler__dot')
        .data(data, labelId);
      ...
      dot
        .attr('cx', compose(halfPixel, props.x))
        .attr('cy', compose(halfPixel, props.y))
        .attr('r', 3.5)
        .attr('fill', props.color);
      ...
      var textSelection = selection$$1.selectAll('.sszvis-ruler__label, .sszvis-ruler__label-outline')
        .attr('transform', function(d) {
          var x = crispX(d);
          var y = crispY(d);
...
var tooltip = function() {
  ...
  return component()
    ...
    .prop('renderInto')
    .prop('visible', functor).visible(false)
    .renderSelection(function(selection$$1) {
      var props = selection$$1.props();
...
var tooltipRenderer = function() {
  return component()
    .prop('header')
    .prop('body')
    .prop('orientation', functor).orientation('bottom')
    .prop('dx', functor).dx(1)
    .prop('dy', functor).dy(1)
    .prop('opacity', functor).opacity(1)
    .renderSelection(function(selection$$1) {
      var tooltipData = selection$$1.datum();
      var props = selection$$1.props();
      ...
      var tooltip = selection$$1.selectAll('.sszvis-tooltip')
        .data(tooltipData);
      ...
      selection$$1.selectAll('.sszvis-tooltip')
...
var fitTooltip = function(defaultVal, bounds) {
...
var bar = function() {
  return component()
    ...
    .render(function(data) {
      ...
      var bars = selection$$1.selectAll('.sszvis-bar')
        .data(data);
...
var dot = function() {
  return component()
    ...
    .render(function(data) {
      ...
      var dots = selection$$1.selectAll('.sszvis-circle')
        .data(data);
...
var groupedBars = function() {
  return component()
    ...
    .prop('fill')
    .prop('stroke')
    .prop('defined', functor).defined(true)
    .render(function(data) {
      var selection$$1 = d3.select(this);
      var props = selection$$1.props();
      ...
      var groups = selection$$1.selectAll('g.sszvis-bargroup')
        .data(data);
...
var line$2 = function() {
  ...
  return component()
    ...
    .prop('valuesAccessor').valuesAccessor(identity)
    .prop('transition').transition(true)
    .render(function(data) {
      var selection$$1 = d3.select(this);
      var props = selection$$1.props();
      ...
      var line$$1 = d3.line()
        .defined(props.defined !== undefined ? props.defined : compose(not(isNaN), props.y))
        .x(props.x)
        .y(props.y);
      ...
      var path = selection$$1.selectAll('.sszvis-line')
        .data(data, props.key);
      ...
      path
        .attr('d', compose(line$$1, props.valuesAccessor))
        .style('stroke', props.stroke)
...
var pie = function() {
  return component()
    ...
    .render(function(data) {
      ...
      var segments = selection$$1.selectAll('.sszvis-path')
        .each(function(d, i) {
...
var pyramid = function() {
  return component()
    .prop('barHeight', functor)
    .prop('barWidth', functor)
    .prop('barPosition', functor)
    .prop('barFill', functor).barFill('#000')
    ...
    .render(function(data) {
      ...
      selection$$1.selectGroup('left')
        .datum(props.leftAccessor(data))
        .call(leftBar);

      selection$$1.selectGroup('right')
        .datum(props.rightAccessor(data))
        .call(rightBar);

      selection$$1.selectGroup('leftReference')
        .datum(props.leftRefAccessor ? [props.leftRefAccessor(data)] : [])
        .call(leftLine);

      selection$$1.selectGroup('rightReference')
...
function lineComponent() {
  return component()
    ...
    .render(function(data) {
      ...
      var line$$1 = selection$$1.selectAll('.sszvis-pyramid__referenceline')
        .data(data);
...
var sankey = function() {
  return component()
    ...
    .prop('nodeThickness')
    .prop('nodePadding')
    .prop('columnPadding', functor)
    .prop('columnLabel', functor).columnLabel('')
    .prop('columnLabelOffset', functor).columnLabelOffset(0)
    .prop('linkCurvature').linkCurvature(0.5)
    .prop('nodeColor', functor)
    .prop('linkColor', functor)
    .prop('linkSort', functor).linkSort(function(a, b) { return a.value - b.value; }) // Default sorts in descending order of value
    .prop('labelSide', functor).labelSide('left')
    .prop('labelSideSwitch')
    .prop('labelOpacity', functor).labelOpacity(1)
    .prop('labelHitBoxSize').labelHitBoxSize(0)
    .prop('nameLabel').nameLabel(identity)
    .prop('linkSourceLabels').linkSourceLabels([])
    .prop('linkTargetLabels').linkTargetLabels([])
    .prop('linkLabel', functor)
    .render(function(data) {
      var selection$$1 = d3.select(this);
      var props = selection$$1.props();
      ...
      var barGroup = selection$$1.selectGroup('nodes')
        .datum(data.nodes);
      ...
      // Render the links
      var linksGroup = selection$$1.selectGroup('links');
      ...
      var linkLabelsGroup = selection$$1.selectGroup('linklabels');
      ...
      var nodeLabelsGroup = selection$$1.selectGroup('nodelabels');
...
var stackedArea = function() {
  return component()
    ...
    .render(function(data) {
      ...
      var paths = selection$$1.selectAll('path.sszvis-path')
        .data(data, props.key);
...
var stackedAreaMultiples = function() {
  return component()
    .prop('x')
    .prop('y0')
    .prop('y1')
    ...
    .render(function(data) {
      ...
      paths
        .attr('d', compose(areaGen, props.valuesAccessor))
        .attr('fill', props.fill)
        .attr('stroke', props.stroke);
    });
};
...
function stackedBarData(order) {
  return function(_stackAcc, seriesAcc, valueAcc) {
    return function(data) {
      ...
      stacks.forEach(function(stack$$1) {
        stack$$1.forEach(function(d) {
          d.series = stack$$1.key;
          d.data = d.data[stack$$1.key][0];
      ...
      stacks.maxValue = d3.max(stacks, function(stack$$1) {
        return d3.max(stack$$1, function(d) { return d[1]; });
      });
...
function stackedBar(config) {
  return component()
    ...
    .render(function(data) {
      ...
      var groups = selection$$1.selectAll('.sszvis-stack')
        .data(data);
...
var horizontalStackedBarConfig = {
  x: function(props) { return compose(props.xScale, fst); },
  y: function(props) { return compose(props.yScale, stackAcc); },
...
var verticalStackedBarConfig = {
  x: function(props) { return compose(props.xScale, stackAcc); },
  y: function(props) { return compose(props.yScale, snd); },
  ...
  return function(data) {
    var sides = cascade()
      ...
      .map(function(rows) {
        ...
        stacks.forEach(function(stack$$1, i) {
          stack$$1.forEach(function(d, row) {
...
function stackedPyramid() {
  return component()
    ...
    .render(function(data) {
      ...
      var leftBar = bar()
        .x(function(d){ return -SPINE_PADDING$1 - props.barWidth(d[1]); })
        .y(compose(props.barPosition, rowAcc))
        .height(props.barHeight)
        .width(function(d){ return props.barWidth(d[1]) - props.barWidth(d[0]); })
        .fill(compose(props.barFill, dataAcc))
        .tooltipAnchor(props.tooltipAnchor);

      var rightBar = bar()
        .x(function(d){ return SPINE_PADDING$1 + props.barWidth(d[0]); })
        .y(compose(props.barPosition, rowAcc))
        .height(props.barHeight)
        .width(function(d){ return props.barWidth(d[1]) - props.barWidth(d[0]); })
        .fill(compose(props.barFill, dataAcc))
      ...
      var leftLine = lineComponent$1()
        .barPosition(props.barPosition)
        .barWidth(props.barWidth)
        .mirror(true);

      var rightLine = lineComponent$1()
        .barPosition(props.barPosition)
        .barWidth(props.barWidth);
      ...
      selection$$1.selectGroup('leftStack')
        .datum(props.leftAccessor(data))
        .call(leftStack);

      selection$$1.selectGroup('rightStack')
        .datum(props.rightAccessor(data))
        .call(rightStack);
...
function stackComponent() {
  return component()
    ...
    .renderSelection(function(selection$$1) {
      ...
      var stack$$1 = selection$$1.selectAll('[data-sszvis-stack]')
        .data(datum);

      var newStack = stack$$1.enter()
        .append('g')
        .attr('data-sszvis-stack', '');

      stack$$1.exit().remove();

      stack$$1 = stack$$1.merge(newStack);

      stack$$1.each(function(d) {
        d3.select(this)
          .datum(d)
...
function lineComponent$1() {
  return component()
    ...
    .render(function(data) {
      ...
      var line$$1 = selection$$1.selectAll('.sszvis-path')
        .data(data);
...
var sunburst = function() {
  return component()
    ...
    .render(function(data) {
      ...
      var arcs = selection$$1.selectAll('.sszvis-sunburst-arc')
        .each(function(d, i) {
          if (data[i]) {
      ...
      selection$$1.call(arcTooltipAnchor);
    });
};
...
var binnedColorScale = function() {
  return component()
    .prop('scale')
    .prop('displayValues')
    .prop('endpoints')
    .prop('width').width(200)
    .prop('labelFormat').labelFormat(identity)
    .render(function() {
      var selection$$1 = d3.select(this);
      var props = selection$$1.props();
      ...
      props.displayValues.forEach(function(p) {
        var w = barWidth(p) - sum$$1;
        var offset = sum$$1 % 1;
        rectData.push({
          x: Math.floor(circleRad + sum$$1),
          w: w + offset,
          c: props.scale(pPrev),
          p: p
        });
      ...
      rectData.push({
        x: Math.floor(circleRad + sum$$1),
        w: innerRange[1] - sum$$1,
        c: props.scale(pPrev)
      });

      var circles = selection$$1.selectAll('circle.sszvis-legend__circle')
      ...
      var segments = selection$$1.selectAll('rect.sszvis-legend__crispmark')
        .data(rectData);
      ...
      var lines = selection$$1.selectAll('line.sszvis-legend__crispmark')
        .data(lineData);
      ...
      var labels = selection$$1.selectAll('.sszvis-legend__axislabel')
        .data(lineData);
...
var linearColorScale = function() {
  return component()
    ...
    .render(function() {
      ...
      var segments = selection$$1.selectAll('rect.sszvis-legend__mark')
        .data(values);
      ...
      var endCaps = selection$$1.selectAll('circle.ssvis-legend--mark')
        .data(startEnd);
      ...
      var labels = selection$$1.selectAll('.sszvis-legend__label')
        .data(labelText);
...
var ordinalColorScale = function() {
  return component()
    .prop('scale')
    ...
    .render(function() {
      ...
      var groups = selection$$1.selectAll('.sszvis-legend--entry')
        .data(domain);
...
var radius = function() {
  return component()
    .prop('scale')
    .prop('tickFormat').tickFormat(identity)
    .prop('tickValues')
    .render(function() {
      var selection$$1 = d3.select(this);
      var props = selection$$1.props();
      ...
      var maxRadius = range$1(props.scale)[1];

      var group = selection$$1.selectAll('g.sszvis-legend__elementgroup')
        .data([0]);
...
function responsiveProps() {
  ...
  function _responsiveProps(measurement) {
    ...
    return Object.keys(propsConfig).reduce(function(memo, propKey) {
      ...
      var matchedBreakpoint = find(function(bp) { return defined(propSpec[bp.name]); }, matchingBreakpoints);
...
function validatePropSpec(propSpec, breakpointSpec) {
  ...
  if (!defined(propSpec._)) { return false; }
...
var bubble = function() {
  var event$$1 = d3.dispatch('over', 'out', 'click');
  ...
  var anchoredCirclesComponent = component()
    ...
    .prop('radius', functor)
    .prop('fill', functor)
    .prop('strokeColor', functor).strokeColor('#ffffff')
    .prop('strokeWidth', functor).strokeWidth(1)
    .prop('transition').transition(true)
    .render(function() {
      var selection$$1 = d3.select(this);
      var props = selection$$1.props();
      ...
      var anchoredCircles = selection$$1.selectGroup('anchoredCircles')
      ...
      anchoredCircles.enter()
        ...
        .on('mouseover', function(d) {
          event$$1.call('over', this, d.datum);
        })
        .on('mouseout', function(d) {
          event$$1.call('out', this, d.datum);
        })
        .on('click', function(d) {
          event$$1.call('click', this, d.datum);
        })
  ...
  anchoredCirclesComponent.on = function() {
    var value = event$$1.on.apply(event$$1, arguments);
    return value === event$$1 ? anchoredCirclesComponent : value;
  };
...
var mapRendererBase = function() {
  return component()
    .prop('mergedData')
    .prop('geoJson')
    .prop('mapPath')
    .prop('defined', functor).defined(true) // a predicate function to determine whether a datum has a defined value
    .prop('fill', functor).fill(function() { return 'black'; }) // a function for the entity fill color. default is black
    ...
    .render(function() {
      ...
      ensureDefsElement(selection$$1, 'pattern', 'missing-pattern')
      ...
      var mapAreas = selection$$1.selectAll('.sszvis-map__area')
      ...
      selection$$1.selectAll('.sszvis-map__area--undefined')
        .attr('fill', getMapFill);

      // change the fill if necessary
      mapAreas
        .classed('sszvis-map__area--undefined', function(d) { return !defined(d.datum) || !props.defined(d.datum); })
      ...
      var tooltipGroup = selection$$1.selectGroup('tooltipAnchors')
...
var geojson = function() {
  ...
  var geojsonComponent = component()
    ...
    .prop('mapPath')
    .prop('defined', functor).defined(true)
    .prop('fill', functor).fill('black')
    .prop('stroke', functor).stroke('black')
    .prop('strokeWidth', functor).strokeWidth(1.25)
    .prop('transitionColor').transitionColor(true)
    .render(function(data) {
      var selection$$1 = d3.select(this);
      var props = selection$$1.props();
      ...
      var geoElements = selection$$1.selectAll('.sszvis-map__geojsonelement')
        .data(mergedData);
      ...
      selection$$1.selectAll('.sszvis-map__geojsonelement--undefined')
        .attr('fill', getMapFill);

      geoElements
        .classed('sszvis-map__geojsonelement--undefined', function(d) { return !defined(d.datum) || !props.defined(d.datum); })
        .attr('d', function(d) { return props.mapPath(d.geoJson); });
      ...
      selection$$1.selectAll('[data-event-target]')
        .on('mouseover', function(d) {
          event$$1.over(d.datum);
        })
        .on('mouseout', function(d) {
          event$$1.out(d.datum);
        })
        .on('click', function(d) {
          event$$1.click(d.datum);
        });
  ...
  geojsonComponent.on = function() {
    var value = event$$1.on.apply(event$$1, arguments);
    return value === event$$1 ? geojsonComponent : value;
  };
...
var mapRendererHighlight = function() {
  return component()
    ...
    .prop('highlightStrokeWidth', functor).highlightStrokeWidth(2)
    .render(function() {
      var selection$$1 = d3.select(this);
      var props = selection$$1.props();

      var highlightBorders = selection$$1
...
var image = function() {
  return component()
    ...
    .render(function() {
      ...
      var image = selection$$1.selectAll('.sszvis-map__image')
...
var mapRendererPatternedLakeOverlay = function() {
  return component()
    .prop('mapPath')
    .prop('lakeFeature')
    .prop('lakeBounds')
    ...
    .render(function() {
      ...
      // the lake texture
      ensureDefsElement(selection$$1, 'pattern', 'lake-pattern')
        .call(mapLakePattern);

      if (props.fadeOut) {
        // the fade gradient
        ensureDefsElement(selection$$1, 'linearGradient', 'lake-fade-gradient')
          .call(mapLakeFadeGradient);

        // the mask, which uses the fade gradient
        ensureDefsElement(selection$$1, 'mask', 'lake-fade-mask')
          .call(mapLakeGradientMask);
      }

      // generate the Lake Zurich path
      var zurichSee = selection$$1.selectAll('.sszvis-map__lakezurich')
      ...
      var lakePath = selection$$1.selectAll('.sszvis-map__lakepath')
...
var raster = function() {
  return component()
    ...
    .render(function(data) {
      ...
      var canvas = selection$$1.selectAll('.sszvis-map__rasterimage')
        .data([0]);
...
var choropleth = function() {
  ...
  var mapComponent = component()
    ...
    .render(function(data) {
      ...
      selection$$1.call(baseRenderer).call(meshRenderer);

      if (props.withLake) {
        selection$$1.call(lakeRenderer);
      }

      selection$$1.call(highlightRenderer);
      ...
      if (props.anchoredShape) {
        ...
        selection$$1.call(props.anchoredShape);
      }
  ...
  mapComponent.on = function() {
    var value = event$$1.on.apply(event$$1, arguments);
    return value === event$$1 ? mapComponent : value;
  };
...
exports.legendColorOrdinal = ordinalColorScale;
