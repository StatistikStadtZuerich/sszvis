...
import {memoize} from '../sszvis/fn.js';
import {component} from '../sszvis/d3-component.js';
import {swissMapPath, mapRendererBase, mapRendererMesh, mapRendererPatternedLakeOverlay, mapRendererHighlight, prepareMergedGeoData, GEO_KEY_DEFAULT} from '../sszvis/map/index.js';
...
var geoData = {
  ...
  featureData: function() { return feature(rawTopo, rawTopo.objects.stadtkreis); },
  // mesh data - a single line that represents all Swiss borders
  meshData: function() { return mesh(rawTopo, rawTopo.objects.stadtkreis); },
  // the lake zurich feature - shared by all three zurich map types
  lakeFeature: function() { return feature(rawTopo, rawTopo.objects.lakezurich); },
  // lakebounds: the section of the map bounds which lies over the Zürichsee
  lakeBounds: function() { return mesh(rawTopo, rawTopo.objects.stadtkreis_lakebounds); }
};

var getData = memoize(function(key) {
  return geoData[key];
});

var zurichStadtKreiseMapData = {
  featureData: getData('featureData'),
  meshData: getData('meshData'),
  lakeFeature: getData('lakeFeature'),
  lakeBounds: getData('lakeBounds')
};

// export zurichStadtKreiseMapData;
...
export var zurichStadtKreiseBaseMap = function() {
  var meshLayer = mapRendererMesh()
    .geoJson(zurichStadtKreiseMapData.meshData());
...
export var zurichStadtKreise = function() {
  var event = dispatch('over', 'out', 'click');

  var featureGeoJson = zurichStadtKreiseMapData.featureData();
  ...
  var highlight = mapRendererHighlight()
    .geoJson(featureGeoJson);
  ...
  var mapComponent = component()
    ...
    .render(function(data) {
      ...
      var mapPath = swissMapPath(props.width, props.height, featureGeoJson, 'zurichStadtKreise');

      var mergedData = prepareMergedGeoData(data, featureGeoJson, props.keyName);
