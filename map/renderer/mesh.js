...
function mapRendererMesh() {
  return component().prop("geoJson").prop("mapPath").prop("key").key(DEFAULT_KEY).prop("borderColor").borderColor(DEFAULT_BORDER_COLOR) // A function or string for the color of all borders. Note: all borders have the same color
  .prop("strokeWidth").strokeWidth(DEFAULT_STROKE_WIDTH).render(function () {
    ...
    }).data([geoJson]).join("path").classed("sszvis-map__border", true).attr(KEY_ATTRIBUTE, props.key);
...
export { mapRendererMesh as default };
